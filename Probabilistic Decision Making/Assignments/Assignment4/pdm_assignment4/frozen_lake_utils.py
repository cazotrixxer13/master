...
def plot_policy(ax, env, Q_table):
    ...
    ax.pcolormesh(
        np.arange(env.ncol + 1), np.arange(env.nrow + 1), map0, cmap="coolwarm"
    )
    ...
    ax.annotate(
        "S",
        xy=(start_position[0] + 0.35, start_position[1] + 0.65),
        color="black",
        size=20,
    )
    ax.annotate(
        "G",
        xy=(goal_position[0] + 0.35, goal_position[1] + 0.65),
        color="black",
        size=20,
    )
...
def plot_frozenlake_policy_iteration_results(
    policy_iteration, gamma, test_rewards, v_star, q_star, savefig=False
):
    plot_frozenlake_results(
        "Policy Iteration",
        q_star,
...
def plot_frozenlake_results(
    ...
    plt.tight_layout()
    if savefig:
        FIG_ROOT = Path("figures_task2")
        FIG_ROOT.mkdir(parents=True, exist_ok=True)
        plt.savefig(
            FIG_ROOT / f'FrozenLake_{algo_name.replace(" ", "_")}_gamma_{gamma}.pdf'
        )
    else:
        plt.show()
